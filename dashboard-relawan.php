...
$notif_count = $conn->query("SELECT COUNT(*) FROM notifikasi WHERE user_id=$user_id AND status='belum_terbaca'")->fetch_row()[0];
...
$evaluasi = [];
if ($status === 'diterima') {
    $q = $conn->query("SELECT e.*, k.nama_kegiatan, k.tanggal, ek.catatan_umum, ek.nilai_akhir  
                       FROM evaluasi e 
                       JOIN kegiatan k ON e.id_kegiatan = k.id 
                       LEFT JOIN evaluasi_keseluruhan ek ON ek.id_kegiatan = e.id_kegiatan 
    ...
    <main>
        ...
        <div class="card" id="koordinator-card" style="margin-bottom: 40px;">
            ...
            <div class="card-footer" id="koordinator-footer" style="display: block;">
                <a href="koordinator-dashboard.php" class="detail-button">Lihat Semua Tugas Koordinator</a>
            </div>
        </div>
        <?php endif; ?>
                ...
                <div class="card-body" id="evaluasi-content">
                    ...
                    ?>
                        <p>
                            ...
                            <strong>Kegiatan Terbaru:</strong> <?= safe($latest['nama_kegiatan']) ?><br>
                            <strong>Umpan Balik:</strong> <?= safe(mb_strimwidth($latest['catatan'] ?? '', 0, 50, '...')) ?></br>
                            <strong>Nilai Keseluruhan Kegiatan:</strong> <?= safe($latest['nilai_akhir'] ?? '-') ?>%</br>
                            <strong>Catatan Umum:</strong> <?= safe(mb_strimwidth($latest['catatan_umum'] ?? '', 0, 50, '...')) ?></br>
                        </p>
                        <div class="card-footer">
